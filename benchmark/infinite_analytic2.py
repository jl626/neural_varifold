...
def main(seed=0, mode='modelnet40', use_graph=False):
  ...
  if mode == 'modelnet40':
    ...
    for i in range(10):
      idx = onp.argwhere(y_label==i)[:,0]
      sample = onp.random.choice(len(idx),5,replace=False)
      new_train.append(x_train[idx[sample]])
      new_label.append(y_train[idx[sample]])
      #new_graph.append( graph1[idx[sample]])
    # sampled train
    x_train = onp.concatenate(new_train,0)[:,:1024,:]#.reshape(10,1024,2,3).transpose((0,2,1,3))
    ...
    x_test = data['x_test'][:920,:1024,:]#.reshape(920,1024,2,3).transpose((0,2,1,3))
