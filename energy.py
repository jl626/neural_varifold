...
class tangent_kernel_pts(nn.Module):
    def __init__(self, num_layers=1,weight=1.0, bias=0.05, dim_size = 3, mode='NTK1'):
        super(tangent_kernel_pts, self).__init__()
        self.num_layers = num_layers
        self.weight = weight
        self.bias = bias
        self.dim_size = dim_size
    ...
    def ntk_fn(self,x1,x2,types='pts'):
        # in case of point clouds normal directions may not be accurate or even completely opposite
        # to alleviate matter, signs are removed by applying quare then square root )
        # initial covariance & NTK kernel
        if types == 'pts':
            K = torch.matmul(x1,x2.T)/self.dim_size
            ntk = torch.matmul(x1,x2.T)/self.dim_size
        else:
            K   = torch.sqrt(torch.matmul(x1,x2.T)**2)/self.dim_size
            ntk = torch.sqrt(torch.matmul(x1,x2.T)**2)/self.dim_size            
        # diagonal initial
        diag_x = torch.sum(x1**2, 1)/self.dim_size
        diag_y = torch.sum(x2**2, 1)/self.dim_size
        # N-hidden layer infinite-width neural network 
    ...
    def forward(self,x1,x2):
        #'''
        if self.mode == 'NTK1':
            g1 = self.ntk_fn(x1[:,:3], x2[:,:3],'pts')
            g2 = self.ntk_fn(x1[:,3:], x2[:,3:],'nor')
            g = g1*g2
        elif self.mode == 'NTK2':
            g = self.ntk_fn(x1, x2)
        return g
